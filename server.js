...
const couponUseSchema = new mongoose.Schema({ user:{type:mongoose.Schema.Types.ObjectId,ref:'User'}, coupon:{type:mongoose.Schema.Types.ObjectId,ref:'Coupon'} },{timestamps:true});
...
app.get('/api/store/products/:id', async (req,res)=>{const p=await Product.findById(req.params.id).populate('category'); if(!p) return res.status(404).json({message:'Not found'}); res.json({id:p._id,img:p.imageUrl,name:p.name,dsc:p.description,price:p.price,rate:p.rating,country:p.country||'Unknown',category:p.category});});
...
app.post('/api/user/cart/apply-coupon', authOnly, async (req,res)=>{const c=await Coupon.findOne({code:req.body.code,isActive:true}); if(!c) return res.status(400).json({message:'Invalid coupon'}); const used=await CouponUse.findOne({user:req.session.userId,coupon:c._id}); if(used) return res.status(400).json({message:'Coupon already used by this user'}); const cart=await Cart.findOne({user:req.session.userId}).populate('items.product'); const subtotal=(cart?.items||[]).reduce((s,i)=>s+i.qty*(i.product?.price||0),0); if(c.minOrderAmount && subtotal<c.minOrderAmount) return res.status(400).json({message:'Min order amount not met'}); if(c.maxUses && c.usageCount>=c.maxUses) return res.status(400).json({message:'Coupon max usage reached'}); const discount=c.discountType==='percentage'?(subtotal*c.discountValue/100):c.discountValue; await CouponUse.create({user:req.session.userId,coupon:c._id}); c.usageCount=(c.usageCount||0)+1; await c.save(); res.json({discount});});
